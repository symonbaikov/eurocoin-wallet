# 🚀 Быстрая настройка внутреннего баланса

## Минимальный набор переменных для работы

### Шаг 1: Генерация секретного ключа

```bash
openssl rand -hex 32
```

Скопируйте результат и добавьте в `.env.local`:

```env
INTERNAL_BALANCE_SIGNING_SECRET=<результат_команды>
```

### Шаг 2: Настройка кошелька казначейства (для автоматических выплат)

**Вариант A: Реальные выплаты**

1. Создайте новый кошелек в MetaMask (НЕ используйте основной!)
2. Экспортируйте приватный ключ
3. Пополните кошелек токенами
4. Добавьте в `.env.local`:

```env
TREASURY_PRIVATE_KEY=0x<ваш_приватный_ключ>
TREASURY_RPC_URL=https://mainnet.infura.io/v3/YOUR_KEY
```

**Вариант B: Тестовый режим (dry-run)**

Оставьте эти переменные пустыми или не указывайте их:

```env
# TREASURY_PRIVATE_KEY=
# TREASURY_RPC_URL=
```

В этом режиме скрипт будет работать без реальных выплат в блокчейн.

### Шаг 3: Настройка Telegram бота (опционально, для команды /credit)

1. Создайте бота через @BotFather в Telegram
2. Получите API ключ
3. Узнайте свой Chat ID (отправьте `/myid` боту)
4. Добавьте в `.env.local`:

```env
TELEGRAM_API_KEY=<ваш_api_ключ>
TELEGRAM_ADMIN_CHAT_ID=<ваш_chat_id>
TELEGRAM_ALLOWED_USER_ID=<ваш_chat_id>
```

---

## 📋 Полный список переменных

### 🔴 Обязательные (минимум для работы)

```env
INTERNAL_BALANCE_SIGNING_SECRET=<сгенерированный_ключ>
```

### 🟡 Для автоматических выплат

```env
TREASURY_PRIVATE_KEY=0x<приватный_ключ>
TREASURY_RPC_URL=https://mainnet.infura.io/v3/YOUR_KEY
```

### 🟢 Рекомендуемые (имеют значения по умолчанию)

```env
TREASURY_WORKER_ID=treasury-bot
TREASURY_BATCH_SIZE=5
TREASURY_TX_CONFIRMATIONS=1
INTERNAL_WITHDRAW_DAILY_LIMIT=1000
INTERNAL_WITHDRAW_MONTHLY_LIMIT=5000
TREASURY_BLOCKED_ADDRESSES=
```

### 🔵 Для Telegram бота (опционально)

```env
TELEGRAM_API_KEY=
TELEGRAM_ADMIN_CHAT_ID=
TELEGRAM_ALLOWED_USER_ID=
```

---

## ✅ Проверка настроек

После заполнения всех переменных:

```bash
# Проверьте наличие обязательных переменных
grep -E "INTERNAL_BALANCE_SIGNING_SECRET" .env.local

# Если используете автоматические выплаты:
grep -E "TREASURY_PRIVATE_KEY|TREASURY_RPC_URL" .env.local
```

---

## 🎯 Что дальше?

1. **Перезапустите сервер** после добавления переменных
2. **Протестируйте начисление баланса:**
   - Через Telegram бота: `/credit`
   - Или через API (см. `docs/admin-balance-guide.md`)
3. **Проверьте отображение баланса** на сайте
4. **Протестируйте вывод** через форму на сайте

---

## 📚 Подробная документация

- Полная инструкция: `docs/internal-balance-env-setup.md`
- Статус функционала: `docs/internal-balance-status-report.md`
- Использование Telegram бота: `docs/telegram-balance-credit-guide.md`
- API документация: `docs/admin-balance-guide.md`
