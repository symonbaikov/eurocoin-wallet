...
bot.action(/^status_(.+)_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    // Edit message to show confirmation (simpler than updating buttons)
    try {
      const message = ctx.callbackQuery.message;
      if (message && "text" in message) {
        await ctx.editMessageText(
          `${message.text}\n\n✅ *Обновлено:* ${stageLabels[newStage] || newStage}`,
          { parse_mode: "Markdown" }
        );
      }
    } catch (editError) {
      // If edit fails, it's okay - user already got callback answer
      console.log("[telegram-webhook] Could not edit message:", editError);
    }
  } catch (error) {
